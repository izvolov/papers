...
1. [Динамические неоднородные контейнеры](#dynamic-heteroheneous-containers)
2. [Динамический кортеж](#dynamic-tuple)
3. [Хранение данных](#data-structure)
4. [Обработчики](#manager)
5. [Доступ к данным](#access)
6. [Время жизни и безопасность исключений](#safety)
7. [Прочие проблемы](#problebs)
8. [Замеры производительности](#benchmark)
9. [Ссылки](#links)

<anchor>dynamic-heteroheneous-containers</anchor>
## [Динамические неоднородные контейнеры](#contents)

Существует несколько <abbr title="Техниками я их называю потому, что получившийся контейнер формально является динамическим однородным. Тем не менее, они имитируют желаемый эффект.">техник</abbr> получения динамического неоднородного контейнера. Вот тройка, пожалуй, наиболее распространённых из них:
...
1.  Неплотная упаковка
    ...
    Как и в случае с массивом указателей, объекты такого массива разбросаны по памяти (в общем случае это не так, потому что может использоваться <abbr title="Объект, размер которого не превышает размер указателя, можно хранить в той же области памяти, что и сам указатель">*оптимизация малых объектов*</abbr>, но для достаточно больших объектов это всегда верно).

<anchor>dynamic-tuple</anchor>
## [Динамический кортеж](#contents)
...
Ну что ж, <abbr title="Давно пора!">приступим к самому интересному</abbr>.

<anchor>data-structure</anchor>
## [Хранение данных](#contents)
...
По итогам <abbr title="Подсмотрел в стандартную библиотеку">долгих размышлений и взвешиваний преимуществ и недостатков</abbr>, недостатки первого варианта перевешивают, и выбор падает на меньшее зло — обобщённый обработчик.
...
## [Время жизни и безопасность исключений](#contents)
...
<anchor>problebs</anchor>
## [Прочие проблемы](#contents)
...
## [Замеры производительности](#contents)
