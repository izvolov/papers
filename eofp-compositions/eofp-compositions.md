...
Напишем функцию<a name="overload-orig" href="#overload-foot"><sup>[1]</sup></a>, которая возвращает размер контейнера:
```cpp
...
Эту проблему можно обойти, если объявить `size` не функцией, а функциональным объектом<a name="functional-orig" href="#functional-foot"><sup>[2]</sup></a>:
```cpp
struct size_fn
{
    template <typename Container>
...
1. <a name="overload-foot" href="#overload-orig">Перегрузку с массивами для простоты опустим.↩︎</a>
2. <a name="functional-foot" href="#functional-orig">Каждый раз, когда функциональный объект называют функтором, страдает какой-нибудь математик. Пожалейте математиков.↩︎</a>
