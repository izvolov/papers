...
> Далее последует описание работы `any` и концепции стирания типа. Читатель, знакомый с этими сущностями, может [перейти непосредственно к сути](#dynamic-tuple-extends-any).
...
> К нижеследующему коду не следует относиться слишком серьёзно — это схематичная реализация. За настоящей реализацией следует обратиться к [более авторитетному источнику](https://github.com/gcc-mirror/gcc/blob/master/libstdc%2B%2B-v3/include/experimental/any).
...
<a name="dynamic-tuple-extends-any"></a>
...
Есть два основных варианта<a title="Уведите детей и беременных от экранов" name="polimorphic-worker-orig" href="#polimorphic-worker-foot"><sup>[4]</sup></a>:
...
По итогам долгих размышлений и взвешиваний преимуществ и недостатков<a name="thoughts-orig" href="#thoughts-foot"><sup>[5]</sup></a>, недостатки первого варианта перевешивают, и выбор падает на меньшее зло — обобщённый обработчик.
...
![График: контейнеры размера 10](containers-of-size-10.png)
![График: контейнеры размера 50](containers-of-size-50.png)
![График: контейнеры размера 100](containers-of-size-100.png)
![График: контейнеры размера 200](containers-of-size-200.png)
![График: контейнеры размера 500](containers-of-size-500.png)
![График: контейнеры размера 1000](containers-of-size-1000.png)
...
4. <a name="polimorphic-worker-foot" href="#polimorphic-worker-orig">На самом деле, есть и третий вариант: полиморфные обработчики-классы. Но этот вариант нещадно отметается как самый тормознутый.↩︎</a>
5. <a name="thoughts-foot" href="#thoughts-orig">Подсмотрел в стандартную библиотеку↩︎</a>
